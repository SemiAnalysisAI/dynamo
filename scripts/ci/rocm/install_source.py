...
def main():
    ...
    require(re.fullmatch("[0-9a-f]{40}", args.source_sha), "Invalid source SHA")
    require(re.fullmatch("[0-9a-f]{64}", args.archive_sha256), "Invalid archive SHA")
    require(os.environ.get("SLURM_JOB_ID"), "Build requires a Slurm allocation")
    contract_path = Path(__file__).with_name("contract.json")
    contract = json.loads(contract_path.read_text())
    require(args.base_uri == contract["base_uri"], "Base image contract mismatch")
    ...
    require(
        json.loads(before)["vllm"]["version"] == contract["tools"]["vllm"],
        "Base vLLM version differs from the contract",
    )
    ...
    require(rust == contract["tools"]["rust"], "Rust toolchain contract mismatch")
    url = (
    ...
    with (args.manifest.parent / "cargo-features.log").open("w") as log:
        run(
            "cargo",
            "tree",
            "--locked",
    ...
    require(
        json.loads(before) == json.loads(after), "Base engine version/origin changed"
    )
    ...
    for name in ("nats", "etcd"):
        ...
        require(
            ...
            f"{name} version differs from the source container context",
        )
        if name == "nats":
    ...
    run(
        PYTHON,
        "/results/controller/slurm_verify.py",
        "build-manifest",
        "--run-dir",
