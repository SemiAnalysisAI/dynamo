...
class ClientTests(unittest.TestCase):
    ...
    def test_staging_transfer_checksum_failure_leaves_no_run(self):
        ...
        result = subprocess.run(
            [sys.executable, "-c", client.STAGE, "local-one", "0" * 64],
        ...
        self.assertIn(b"Transfer checksum mismatch", result.stderr)
        self.assertFalse((self.root / "dynamo-rocm-ci/runs/local-one").exists())

    def test_existing_remote_run_never_overwritten(self):
        target = self.root / "dynamo-rocm-ci/runs/local-one"
