...
def command_timeout(deadline=None):
    if deadline is None:
        return 20
    remaining = deadline - time.monotonic()
    if remaining <= 0:
        raise TimeoutError("Scheduler operation deadline exhausted")
    return min(20, remaining)


def validate_partition(name):
...
def recover(run, req, deadline=None):
    result = command(
        [
            "squeue",
            "--noheader",
    ...
    for job_id in set(result.stdout.split()):
        parse_job_id(job_id)
        if inspect_job(run, req, job_id, timeout=command_timeout(deadline)):
...
def cancel(run, req, job_id, deadline=None):
    evidence = inspect_job(run, req, job_id, timeout=command_timeout(deadline))
    if evidence is None:
        raise RuntimeError("cannot verify job identity for cancellation")
    if evidence["state"] not in TERMINAL:
        result = command(["scancel", str(job_id)], timeout=command_timeout(deadline))
        atomic_json(
            run / "cancel.json",
...
def cancelled_before_submission(run):
    """Called under the controller lock before any sbatch process is created."""
    if not (run / "cancel-request.json").exists():
        return False
    atomic_json(
        run / "controller-result.json",
        {"status": "cancelled", "reason": "cancelled before submission"},
    )
    return True


def await_submission(run, req, deadline):
    """Reconcile an in-flight sbatch while its durable cancellation remains armed."""
    while True:
        if (run / "receipt.json").exists():
            return read_json(run / "receipt.json")
        if not (run / "submission-intent.json").exists():
            return None
        receipt = recover(run, req, deadline=deadline)
        if receipt:
            return receipt
        if (run / "controller-result.json").exists():
            return None
        time.sleep(min(2, command_timeout(deadline)))
...
def idle_gpu_node(record, preferred_partition="compute-1"):
    """Recognize healthy idle GPU nodes with enough requested resources."""
    states = set(record.get("State", "").split("+"))
    if "IDLE" not in states or states - {"IDLE", "DYNAMIC_NORM"}:
        return False
    if preferred_partition not in record.get("Partitions", "").split(","):
        return False
    try:
...
def monitor(run):
    ...
    with lock(run):
        if (run / "submission-intent.json").exists() or cancelled_before_submission(
            run
        ):
            return
    started = time.time()
    args = [
        ...
        "--chdir=" + str(run),
        str(run / "controller" / "slurm.sbatch"),
        str(run),
    ]
    ...
    try:
        ...
        # Finalization uses this same lock. It either prevents submission, or
        # observes an in-flight submission and waits for a verifiable job ID.
        with lock(run):
            if (run / "submission-intent.json").exists() or cancelled_before_submission(
                run
            ):
        ...
        while selector.get_map() or process.poll() is None:
            ...
            if now - last_check >= 5:
                ...
                cancellation_requested = (run / "cancel-request.json").exists()
                if job_id is None and (cancellation_requested or now - started > 60):
                    recovered = recover(run, req)
                    if recovered:
                        job_id = recovered["job_id"]
                if job_id and cancellation_requested and not cancellation_sent:
        ...
        # Preserve unexpected daemon failures and release the allocation before raising.
        diagnostics = []
        if job_id is None:
            try:
                recovered = recover(run, req)
...
def main():
    ...
    with lock(run):
        ...
        if (
            args.action == "start"
            and not (run / "monitor.json").exists()
            and not cancelled_before_submission(run)
        ):
        ...
        if receipt:
            ...
            while True:
                ...
                if (
                    ...
                    break
                time.sleep(2)
        elif (run / "submission-intent.json").exists() and not (
            run / "controller-result.json"
        ).exists():
            atomic_json(
                run / "controller-result.json",
